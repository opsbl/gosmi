...
func (t SmiNode) getRow() (row *types.SmiNode) {
	...
	case types.NodeTable:
		row = t.instance.smiInst.GetFirstChildNode(t.smiNode)
		if row == nil {
			return
		}
	default:
...
func (t SmiNode) GetRow() (row SmiNode) {
	...
	return CreateNode(t.instance, smiRow)
}
...
func (t SmiNode) GetColumns() (columns map[string]SmiNode, columnOrder []string) {
	...
	for smiColumn := t.instance.smiInst.GetFirstChildNode(row); smiColumn != nil; smiColumn = t.instance.smiInst.GetNextChildNode(smiColumn) {
		if smiColumn.NodeKind != types.NodeColumn {
			// TODO: error
			return
		}
		column := CreateNode(t.instance, smiColumn)
...
func (t SmiNode) GetAugment() (row SmiNode) {
	...
	smiRow = t.instance.smiInst.GetRelatedNode(smiRow)
	if smiRow == nil {
		return
	}
...
func (t SmiNode) GetIndex() (index []SmiNode) {
	...
	if row.IndexKind == types.IndexAugment {
		row = t.instance.smiInst.GetRelatedNode(row)
		if row == nil {
			return
		}
	...
	for smiElement := t.instance.smiInst.GetFirstElement(row); smiElement != nil; smiElement = t.instance.smiInst.GetNextElement(smiElement) {
		smiColumn := t.instance.smiInst.GetElementNode(smiElement)
		if smiColumn == nil {
			// TODO: error
			return
		}
		...
		index = append(index, CreateNode(t.instance, smiColumn))
	}
	return
}
