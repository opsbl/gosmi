...
func (x *Module) GetObject(name types.SmiIdentifier) *Object {
	...
	wellKnown := x.Handle.Modules.Get(WellKnownModuleName)
	if wellKnown != nil {
	...
	i.Used = true
	module, err := x.Handle.GetModule(i.Module.String())
	if err != nil {
		return nil
	}
...
func (x *Module) SetPrefixNode(n *Node) {
	...
	if len(n.Oid) < len(x.PrefixNode.Oid) {
		nodePtr := x.Handle.FindNodeByOid(len(n.Oid), x.PrefixNode.Oid)
		if nodePtr == nil {
	...
	for i, subId := range x.PrefixNode.Oid {
		if subId != n.Oid[i] {
			x.PrefixNode = x.Handle.FindNodeByOid(i, x.PrefixNode.Oid)
			return
		}
	}
}
...
func (h *Handle) FindModuleByName(modulename string) *Module {
	return h.Modules.GetName(modulename)
}

func (h *Handle) GetModuleFile(name string) (string, io.ReadCloser, error) {
	if name == "" {
...
func (h *Handle) GetModule(name string) (*Module, error) {
	module := h.FindModuleByName(name)
	if module != nil {
		return module, nil
	}
...
func (h *Handle) LoadModule(name string) (*Module, error) {
	if h == nil {
		return nil, errors.New("SMI not initialized")
	}
	if h.loading == nil {
		h.loading = make(map[types.SmiIdentifier]bool)
	}
	rootCall := len(h.loading) == 0
	requestedKey := types.SmiIdentifier(name)
	if h.loading[requestedKey] {
		err := fmt.Errorf("circular import detected for module %s", name)
		if h.loadErr == nil {
			h.loadErr = err
		}
		return nil, err
	}
	h.loading[requestedKey] = true
	defer delete(h.loading, requestedKey)

	//log.Printf("%s: Loading", name)
	path, f, err := h.GetModuleFile(name)
	if err != nil {
		if h.loadErr == nil {
			h.loadErr = err
		}
	...
	parsedKey := in.Name
	if h.loading[parsedKey] && parsedKey != requestedKey {
		err := fmt.Errorf("circular import detected for module %s", parsedKey)
		if h.loadErr == nil {
			h.loadErr = err
		}
		return nil, err
	}
	h.loading[parsedKey] = true
	defer delete(h.loading, parsedKey)
	//log.Printf("%s: Parsed", name)
	out, err := h.BuildModule(path, in)
	if err != nil {
		if h.loadErr == nil {
			h.loadErr = err
		}
...
func (h *Handle) BuildModule(path string, in *parser.Module) (*Module, error) {
	...
	for _, t := range in.Body.Types {
		...
		parentType := out.Handle.GetBaseTypeFromSyntax(syntax)
		if parentType == nil {
			...
			if len(objType.Index) > 0 {
				...
				if syntax.SubType != nil {
					...
					if currType.BaseType == types.BaseTypeBits {
						if parentType == out.Handle.TypeBits {
							currType.Name = "Bits"
						} else {
